...
#### 面试补充：Context 可以用来做什么？

`Context`最核心的用途是传递请求生命周期，而不是替代函数参数或全局变量。

常见用途可以总结为：

1. **取消信号传播**：一个请求取消后，下游 RPC、数据库查询、后台 goroutine 都能一起停止。
2. **超时和截止时间控制**：防止下游调用无限等待，避免 goroutine、连接和线程池资源被拖住。
3. **请求级元数据传递**：例如 trace id、request id、用户身份、租户信息，但不应该传大对象或可选业务参数。
4. **跨 API 边界统一约束**：HTTP handler、RPC client、数据库 driver 都可以接收 `context.Context`，让取消和超时在链路里传递。

使用注意事项：

1. `Context`通常作为函数第一个参数传递，命名为 `ctx`。
2. 不要把 `Context` 存进结构体长期持有，除非这个结构体本身就是请求级对象。
3. `WithCancel`、`WithTimeout`、`WithDeadline` 返回的 `cancel` 要及时调用，释放定时器等资源。
4. `WithValue` 只适合传请求级元数据，不适合传业务必需参数。

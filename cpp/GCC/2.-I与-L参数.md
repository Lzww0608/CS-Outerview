...
### 1.1 `#include` 的搜索顺序：尖括号和双引号

`#include` 是预处理阶段完成的文本包含。它不负责链接符号，也不会检查函数最终有没有实现，只是把被包含文件的内容展开到当前翻译单元中。
...
1. `#include "xxx.h"`：通常先在当前源文件所在目录或编译器定义的“当前 include 目录”查找，再查找 `-I` 指定目录和系统目录。
2. `#include <xxx.h>`：通常直接查找 `-I` 指定目录和系统目录，不优先查当前源文件目录。
3. 具体细节和编译器有关，但工程约定通常是：项目内部头文件用双引号，标准库和第三方稳定安装头文件用尖括号。

头文件搜索顺序经常影响同名文件解析。例如项目里有一个 `string.h`，如果错误地写成 `#include "string.h"`，可能会优先包含项目本地文件，而不是 C 标准库头文件，导致非常隐蔽的编译错误。

工程上要注意：

1. 头文件目录要通过 `-I`、构建系统的 `include_directories` 或 `target_include_directories` 明确声明。
2. 公共头文件中不要依赖“恰好被别的头文件提前包含”，应该自包含，即单独 include 后也能编译。
3. 头文件顺序建议从本文件对应头文件、C/C++ 标准库、第三方库、项目内头文件依次排列，便于暴露自包含问题。
4. 头文件需要使用 include guard 或 `#pragma once` 防止重复包含。

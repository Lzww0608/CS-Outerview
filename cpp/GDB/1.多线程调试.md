...
`bt` 输出当前线程调用栈；`backtrace full` 会额外打印每个栈帧中的局部变量，适合排查死锁、core dump 和异常分支。线上服务不方便交互时，也可以先用：
...
`pstack` 可以快速打印线程栈，`gcore` 生成 core 文件后再用 `gdb <program> core` 离线分析，减少长时间挂住线上进程。

...
C++ 中的隐式转换是指编译器在需要某种类型时，自动把表达式转换成目标类型。类类型里常见来源有两类：

1. **转换构造函数**：非 `explicit` 的单参数构造函数，或者除第一个参数外其余参数都有默认值的构造函数。
2. **类型转换运算符**：例如 `operator bool()`、`operator int()`。
...
这段代码可以通过编译。原因是 `A(int)` 不是 `explicit`，编译器可以用整数 `2` 构造一个临时对象 `A{2}`，再把这个临时对象绑定到 `const A&` 参数上。临时对象的生命周期会延长到本次函数调用结束，因此在 `print` 内访问 `a._a` 是有效的。
...
1. 可能让本不该通过的调用通过编译，隐藏业务错误。
2. 可能引起重载决议歧义。
3. 可能产生临时对象，带来额外构造成本。
4. 多步转换链会降低代码可读性。

工程上建议：凡是“一个参数构造对象”但语义上不是自然类型转换的构造函数，都优先加 `explicit`。例如 `String(size_t n)`、`File(int fd)`、`Socket(int fd)`、`Duration(int seconds)` 这类构造通常不希望被随意隐式调用。

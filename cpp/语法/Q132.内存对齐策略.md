...
内存对齐是指对象地址要满足某个倍数约束，例如 `int` 常要求 4 字节对齐，`double` 常要求 8 字节对齐。编译器会在结构体成员之间插入 padding，使每个成员和整个对象数组都满足对齐要求。

常见结构体规则：

1. 每个成员按自身对齐要求放置，必要时在前面填充 padding。
2. 结构体整体对齐值通常是所有非静态成员对齐值的最大值。
3. 结构体总大小要补齐到整体对齐值的整数倍，保证数组中下一个元素也满足对齐。
4. 非静态成员参与对象布局；普通成员函数、静态成员函数、静态数据成员不计入单个对象大小。
...
1. CPU 访问自然对齐的数据通常更快。
2. 一些架构对非对齐访问会触发异常或需要多次内存访问。
3. 原子操作、SIMD、cache line 优化通常要求更严格的对齐。
4. 二进制协议、硬件寄存器、磁盘文件格式需要明确布局时，要主动控制 padding。
...
在常见平台上，`int` 大小 4、对齐 4，`char` 大小 1、对齐 1，`short` 大小 2、对齐 2。布局通常是：

```text
offset 0..3 : int a
offset 4    : char b
offset 5    : padding，补齐 short 的 2 字节对齐
offset 6..7 : short c
sizeof(Test) = 8，alignof(Test) = 4
```

如果中间插入了一个未知成员，不应该靠肉眼硬算 `c` 的偏移，应该让编译器计算：

```cpp
#include <cstddef>
#include <iostream>

std::cout << offsetof(Test, c) << '\n';
```

如果确实要通过偏移取到 `c`，只能在确认类型是标准布局、对象生命周期有效、偏移来自 `offsetof` 的前提下做：

```cpp
Test t{1, 'x', 2};
auto* base = reinterpret_cast<char*>(&t);
auto* pc = reinterpret_cast<short*>(base + offsetof(Test, c));
std::cout << *pc << '\n';
```

真实代码里更推荐直接使用 `t.c` 或封装访问函数。通过地址加偏移访问成员强依赖 ABI、对齐和标准布局条件；一旦类有虚函数、继承、访问控制复杂或非标准布局，手写偏移就容易变成未定义行为。

### 0.2 `char + int + short` 的 `sizeof` 怎么算
...
在常见 32/64 位 ABI 下，`char` 大小和对齐都是 1，`int` 大小 4、对齐 4，`short` 大小 2、对齐 2。结构体整体对齐值取最大成员对齐值，也就是 4。

布局通常是：

```text
offset 0    : char a
offset 1..3 : padding，补齐 int 的 4 字节对齐
offset 4..7 : int b
offset 8..9 : short c
offset 10..11 : tail padding，补齐结构体整体 4 字节对齐
sizeof(A) = 12，alignof(A) = 4
```

易错点：

1. 不能简单把成员大小相加成 `1 + 4 + 2 = 7`。
2. 末尾 padding 也算进 `sizeof`，否则 `A arr[2]` 中第二个元素的 `int b` 可能无法对齐。
3. 使用 `#pragma pack(1)` 或编译器 ABI 不同会改变结果，面试默认一般按当前平台自然对齐回答，并说明前提。
...
1. `#pragma pack(1)` 能减少 padding，但可能带来非对齐访问成本，不应该为了省几个字节到处使用。
2. 网络协议和磁盘格式不要直接 `send/write` 整个结构体，除非明确控制字节序、对齐和字段版本。
3. `alignas` 只能提高或保持对齐要求，不能把一个类型压到低于自然对齐。
4. 为了避免伪共享，可以让热点原子变量按 cache line 对齐，例如 `alignas(64)`。
5. 结构体成员顺序会影响 padding，常把大对齐成员放前面以减少空洞。

面试回答可以按“规则 + 方法 + 场景”组织：默认对齐由成员自然对齐和最大对齐决定；需要提高对齐用 `alignas`、GCC/Clang 的 `aligned` 或 MSVC 的 `__declspec(align)`；需要按外部协议压紧布局用 `#pragma pack`。真实项目中还要考虑性能、字节序和跨平台 ABI。

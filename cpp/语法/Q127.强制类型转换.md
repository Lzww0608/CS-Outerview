...
`static_cast` 表达的是“编译期可理解的类型转换”，`reinterpret_cast` 表达的是“把同一段位模式按另一种类型重新解释”。前者仍然在 C++ 类型系统内做相对受约束的转换，后者基本是在绕过类型系统。

| 对比项 | `static_cast` | `reinterpret_cast` |
| --- | --- | --- |
| 主要语义 | 编译期类型转换 | 低级别位模式重新解释 |
| 常见用途 | 数值转换、枚举转换、向上转型、明确安全时的向下转型、`void*` 转回原类型指针 | 不相关指针互转、指针和整数互转、底层协议/内存映射中的特殊处理 |
| 是否改位模式 | 可能做值转换，例如 `double` 到 `int` | 通常不改变底层位模式，只改变解释方式 |
| 类型检查 | 有较多编译期检查 | 检查很少，风险更高 |
| 类继承转换 | 支持相关类型，向下转型无运行时检查 | 不应用于类层次安全转换 |
| 典型风险 | 窄化、错误向下转型导致未定义行为 | 对齐、别名规则、对象生命周期、平台相关行为 |

示例：

```cpp
double d = 3.14;
int x = static_cast<int>(d);  // 值转换，结果是 3

struct A { int a; };
struct B { int b; };

A obj{42};
B* p = reinterpret_cast<B*>(&obj);  // 只是把 A* 当作 B* 看，解引用通常是未定义行为
```

面试回答时可以概括为：能用 `static_cast` 表达的转换，不要用 `reinterpret_cast`；只有在做底层系统编程、序列化缓冲区、硬件寄存器映射、指针整数保存等极少数场景下才考虑 `reinterpret_cast`，并且要明确对齐、生命周期和别名规则。

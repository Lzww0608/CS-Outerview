...
`shared_ptr` 的计数变化发生在“共享所有权变化”时，而不是发生在访问对象成员时。

常见规则：

1. **拷贝构造**：新 `shared_ptr` 和旧 `shared_ptr` 指向同一控制块，强引用计数 `+1`。
2. **拷贝赋值**：通常先增加新目标控制块计数，再减少旧目标控制块计数；如果旧计数减到 0，就析构被管理对象。
3. **析构或 `reset()`**：当前 `shared_ptr` 放弃所有权，强引用计数 `-1`；减到 0 时销毁对象，等弱引用也归零后释放控制块。
4. **按值传参**：形参会由实参拷贝构造，进入函数时强引用计数 `+1`，函数返回形参析构后 `-1`。
5. **按 `const shared_ptr<T>&` 传参**：只借用智能指针对象，不改变引用计数。
6. **移动构造或 `std::move(sp)` 传参**：所有权从源对象转移到目标对象，通常不增加引用计数；源 `shared_ptr` 变为空或处于可析构状态。
...
void byRef(const std::shared_ptr<int>& p) {
    // 只观察 p，不增加引用计数
}

auto p = std::make_shared<int>(1); // use_count = 1
auto q = p;                        // use_count = 2
byValue(p);                        // 调用期间 use_count = 3，返回后回到 2
byRef(p);                          // use_count 仍为 2
byValue(std::move(q));             // 通常不额外 +1，q 被移动后不再拥有对象
```

易错点：

1. `std::move` 本身不移动对象，只把表达式转换成右值；是否转移所有权取决于后续调用的是移动构造还是移动赋值。
2. `use_count()` 只适合调试或理解所有权，不适合用来做并发控制；读取到的值可能马上被其他线程改变。
3. 热路径里频繁按值传递 `shared_ptr` 会造成原子引用计数抖动，应优先传 `T&`、`T*`、`const shared_ptr<T>&`，或者明确生命周期后用 `weak_ptr` 观察。
...
## 6.1 为什么 `shared_ptr` 不是 lock-free

`shared_ptr` 的引用计数通常用原子变量维护，但这不等于 `shared_ptr` 整体就是 lock-free。

原因：

1. **控制块分配不是 lock-free**：创建 `shared_ptr` 时通常要分配控制块，分配器内部可能加锁。
2. **对象析构不是 lock-free**：最后一个强引用释放时会调用析构函数和删除器，删除器里可能做任意逻辑。
3. **弱引用提升需要同步**：`weak_ptr::lock()` 要在对象未销毁时安全增加强引用计数，需要 compare-exchange 等同步。
4. **同一个 `shared_ptr` 变量并发读写需要保护**：C++20 的 `std::atomic<std::shared_ptr<T>>` 提供原子 load/store/exchange，但标准不保证所有实现都是 lock-free，可以用 `is_lock_free()` 查询。
5. **引用计数原子也有缓存一致性成本**：多线程频繁复制同一个控制块，会让强引用计数所在缓存行在 CPU 之间来回同步。

所以面试里可以说：`shared_ptr` 保证的是控制块引用计数更新的线程安全，不保证无锁，也不保证被管理对象线程安全。热路径里如果频繁传递共享所有权，应考虑改成借用引用、对象池、RCU、`intrusive_ptr` 或明确生命周期边界，减少引用计数抖动。

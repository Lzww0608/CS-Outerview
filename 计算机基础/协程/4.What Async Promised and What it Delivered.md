...
OS threads carry overhead: an operating system thread reserves virtual address space for its stack and takes on the order of tens of microseconds to create on modern Linux. Context switches happen in kernel space and burn CPU cycles, and O(n) readiness polling (select, poll) add up at scale. A server handling thousands of concurrent connections and dedicating one thread per connection means thousands of threads each consuming memory and competing for scheduling. The system spends time managing threads that could be better spent doing useful work.
...
Some language designers chose different paths. For example, Go deliberately chose goroutines, accepting a heavier runtime in exchange for no function coloring at all. Java's Project Loom (virtual threads in Java 21) also made a different bet: lightweight threads that look and behave like regular threads, so no code needs to change color. The Loom team explicitly cited function coloring as a problem they wanted to avoid.
